...
func (b *BlockWalker) handleStaticCall(e *expr.StaticCall) bool {
	...
	if !ok {
		b.handleCallArgs(e.Call, e.ArgumentList.Arguments, meta.FuncInfo{})
		return false
	}
...
func (b *BlockWalker) handleNew(e *expr.New) bool {
	...
	className, ok := solver.GetClassName(b.r.st, e.Class)
	if !ok {
		// perhaps something like 'new $class', which should be walked, but
		// could also be an inline class declaration, which isn't handled yet
